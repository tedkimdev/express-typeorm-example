...
import { GetCarsInput, DeleteCarOutput, GetCarsOutput, GetCarOutput,CreateCarInput, CreateCarOutput, UpdateCarInput, UpdateCarOutput } from "../../dtos/car";
...
export class CarController {
  ...
  @Get()
  getCars(@Res() response: Response, @QueryParams() getCarsInput: GetCarsInput): Promise<GetCarsOutput> {
  ...
  @Post()
  createCar(@Res() response: Response, @Body() createCarInput: CreateCarInput): Promise<CreateCarOutput> {
    return handler.createCar(this.carService, this.vehicleService)(response, createCarInput);
  }

  @Put("/:id")
  updateCar(@Param("id") id: string, @Res() response: Response, @Body() updateCarInput: UpdateCarInput): Promise<UpdateCarOutput> {
